# keybase://chat/chia_network.public#chialisp/4896
# https://github.com/Chia-Network/chia-dev-tools/tree/main/cdv/examples
import pytest
...
class TestStandardTransaction:
    ...
    async def make_and_spend_piggybank(self, network, alice, bob, CONTRIBUTION_AMOUNT):
        ...
        # This will use one mojo to create our new piggybank with 1 xch target with bob's puzzle_hash (for cashing out) on the blockchain.
        piggybank_coin = await alice.launch_smart_coin(create_piggybank_puzzle(uint64(1000000000000), bob.puzzle_hash))
    ...
    async def test_piggybank_contribution(self, setup):
        ...
        try:
            ...
            # check the additions from result
            # there has to be an addition with 501 mojo & bob's puzzle_hash
